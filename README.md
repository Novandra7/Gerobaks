...
- **Laravel 12**: REST API, scheduling, dan job pipeline
- **Laravel Sanctum**: Token-based authentication untuk mobile client
- **MySQL / SQLite**: Persistent storage (SQLite default untuk pengembangan lokal)
- **Queue & Events**: Menangani aktivitas asinkron (tracking, notifikasi)
- **Google Gemini AI**: Tetap digunakan oleh client untuk konten AI lokal
...
3. Salin `.env.example` ke `.env` dan sesuaikan kredensial database
4. Jalankan `php artisan key:generate`
5. Jalankan migrasi & seeder: `php artisan migrate --seed`
6. Start server lokal: `php artisan serve`

> **Catatan**: API menggunakan Laravel Sanctum. Pastikan tabel `personal_access_tokens` berhasil dibuat dan seeder berjalan tanpa error agar data demo tersedia untuk aplikasi Flutter.

#### Frontend (Flutter App)

1. Kembali ke root project
2. Salin `.env.example` ke `.env` dan set `API_BASE_URL` ke alamat server backend (contoh Android emulator: `http://10.0.2.2:8000`)
...
NOMINATIM_API_KEY=
MAPS_API_KEY=
QRIS_ID_TAG_26=ID10220000000000098765432101
QRIS_MERCHANT_NAME=GEROBAKS
QRIS_CITY=JAKARTA
