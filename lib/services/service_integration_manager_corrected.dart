...
class ServiceIntegrationManager {
  ...
  Future<void> _initializeCoreServices() async {
    try {
      ...
      // If already initialized, that's okay for testing purposes
      if (e.toString().contains('already been initialized')) {
        print('⚠️ Services already initialized, skipping re-initialization');
        return;
      }
      rethrow;
    }
  ...
  Future<void> _initializeFeatureServices() async {
    try {
      ...
      if (e.toString().contains('already been initialized')) {
        print(
          '⚠️ Feature services already initialized, skipping re-initialization',
        );
        return;
      }
      rethrow;
  ...
  Future<Map<String, dynamic>> _getAdminDashboardData() async {
    try {
      final reportsData = await _reportAdminService.getReports(limit: 10);
      return {
        'users': [], // User management will be handled by UserService
        'reports': reportsData['data'] ?? [],
        'statistics': {
          'total_reports': reportsData['total'] ?? 0,
          'pending_reports': reportsData['pending'] ?? 0,
        },
        'recent_activities': await _getRecentActivities(),
      };
    } catch (e) {
      print('⚠️ Failed to get admin dashboard data: $e');
      return {
        'users': [],
        'reports': [],
        'statistics': {},
  ...
  Future<Map<String, dynamic>> _getMitraDashboardData() async {
    try {
      ...
      final mitraDashboard = await _dashboardBalanceService.getMitraDashboard();

      return {
        'orders': orders,
        'schedule': [], // Schedule service integration pending
        'earnings': {
          'total_earnings': mitraDashboard.totalEarnings,
          'today_earnings': mitraDashboard.todayEarnings,
          'total_orders': mitraDashboard.totalOrders,
          'completed_orders': mitraDashboard.completedOrders,
        },
        'ratings': [], // Rating service integration pending
        'metrics': mitraDashboard,
      };
    } catch (e) {
      print('⚠️ Failed to get mitra dashboard data: $e');
      return {
        'orders': [],
        'schedule': [],
        'earnings': {},
  ...
  Future<Map<String, dynamic>> _getEndUserDashboardData() async {
    try {
      final orders = await _orderService.getMyOrders();
      final balanceSummary = await _dashboardBalanceService.getBalanceSummary();
      final userDashboard = await _dashboardBalanceService.getUserDashboard();

      return {
        'my_orders': orders,
        'balance': {
          'current_balance': balanceSummary.currentBalance,
          'total_credit': balanceSummary.totalCredit,
          'total_debit': balanceSummary.totalDebit,
          'total_transactions': balanceSummary.totalTransactions,
        },
        'notifications': [], // Notification service integration pending
        'tracking': [], // Tracking service integration pending
        'metrics': userDashboard,
      };
    } catch (e) {
      print('⚠️ Failed to get end user dashboard data: $e');
      return {
        'my_orders': [],
        'balance': {},
        'notifications': [],
