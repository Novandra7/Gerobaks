...
/// Tab untuk schedule dengan status: cancelled
/// Menampilkan jadwal yang dibatalkan
/// Fitur: Cancellation reason, Reschedule option
...
class _CancelledTabState extends State<CancelledTab>
  ...
  List<ActivityModel> _getFilteredActivities() {
    // Filter hanya status 'cancelled'
    final cancelledSchedules = _schedules.where((schedule) {
      return schedule['status'] == 'cancelled';
    }).toList();

    // Convert ke ActivityModel
    List<ActivityModel> activities = cancelledSchedules.map((schedule) {
  ...
  Widget _buildEmptyState() {
    String title = 'Tidak ada pickup dibatalkan';
    String subtitle = 'Jadwal yang dibatalkan akan muncul di sini';
